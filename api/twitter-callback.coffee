...
module.exports = (req, res) ->
    ...
    oa.getOAuthAccessToken cacheAuth.token, cacheAuth.token_secret, req.query.oauth_verifier, (error, oauth_access_token, oauth_access_token_secret, results) ->     
        @db.collection 'users', (error, collection) ->
            collection.findOne {uid:parseInt(cacheAuth.uid, 10)}, (error, user) -> 
                if user
                    collection.update {uid:parseInt(cacheAuth.uid, 10)}, {$set:{twitter:{access_token:oauth_access_token, access_secret:oauth_access_token_secret}}}, (error, result) ->
